...
function createMotionComponent(element: string) {
  return function MotionComponent({ children, ...props }: MotionProps) {
    ...
    return React.createElement(element, htmlProps as React.HTMLAttributes<HTMLElement>, children);
  };
}
 ...
 */
export function AnimatePresence({ children }: React.PropsWithChildren<unknown>) {
